...
namespace MtmMath {
    ...
    public:
         ...
         * in the matrix, val is the initial value for the matrix elements and
         * isUpper_ is whether it is upper Rectangular matrix (true means it is)
         */
        MtmMatTriag<T> (size_t m, const T& val=T(), bool isUpper_t=true) :
                                        MtmMatSq<T>(m,0) , is_upper(isUpper_t){
            if(is_upper){
        ...
        MtmMatTriag(const MtmMatTriag& mt) : MtmMatSq<T>(mt) ,
                                                is_upper(mt.is_upper){}
        ...
        virtual void resize(Dimensions dim, const T& val=T()) {
            if(dim.getCol() != dim.getRow()){
                throw MtmExceptions::ChangeMatFail(this->r, this->c,
